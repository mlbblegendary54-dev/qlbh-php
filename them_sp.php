...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($ten) || empty($gia) || empty($soluong)) {
        $thongbao = ['loai'=>'error','noi_dung'=>'Vui lòng điền đầy đủ thông tin!'];
    } else {
        $stmt = $conn->prepare("INSERT INTO sanpham (ten_sp,gia,so_luong,danh_muc) VALUES (?,?,?,?)");
        $stmt->bind_param("sdis", $ten, $gia, $soluong, $danhmuc);
        if ($stmt->execute()) {
            $thongbao = ['loai'=>'success','noi_dung'=>'Thêm sản phẩm thành công!'];
        } else {
            $thongbao = ['loai'=>'error','noi_dung'=>'Có lỗi xảy ra, thử lại!'];
        }
        $stmt->close();
    }
}
...
<head>
<meta charset="UTF-8">
<title>Thêm sản phẩm · ShopManager</title>
<style>
...
.btn-cancel {
    ...
    transition:all .2s; font-family:'Inter',sans-serif; cursor:pointer;
}
...
<aside class="sidebar">
    ...
    <nav class="sidebar-menu">
        ...
        <a href="them_sp.php" class="menu-item active">
    ...
    <div class="form-card">
        ...
        <form method="POST" action="">
            <div class="form-group">
                ...
                <input type="text" name="ten_sp" placeholder="VD: Áo thun nam basic" required>
            </div>
            <div class="form-row">
                <div class="form-group">
                    <label>Giá bán (VNĐ)</label>
                    <input type="number" name="gia" placeholder="150000" min="0" required>
                    <div class="form-hint">Nhập số, không cần dấu chấm</div>
                </div>
                <div class="form-group">
                    <label>Số lượng tồn</label>
                    <input type="number" name="so_luong" placeholder="50" min="0" required>
                </div>
            </div>
            <div class="form-group">
